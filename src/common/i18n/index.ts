...
export interface UseCustomTranslationResponse<Ns extends Namespace, KPrefix>
  ...
  tc: (msg: string, options?: object) => string;
  tca: (msg: string, sep?: string, options?: object) => string;
}
...
> => {
  ...
  return {
    t,
    tc: (msg: string, options) => (msg ? capitalize(t(msg, options)) : msg),
    tca: (msg: string, sep?: string, options?) =>
      msg ? capitalizeAll(t(msg, options), sep) : msg,
